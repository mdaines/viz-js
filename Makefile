...
$(SRCDIR)/lib/common/libcommon-em.bc:
	cd $(SRCDIR)/lib/common; $(EMCC) $(EMFLAGS) -o libcommon-em.bc -I. -I.. -I../.. -I../../.. -I../gvc -I../pathplan -I../cdt -I../graph -I../xdot -I../../../$(EPSRCDIR)/lib -DHAVE_CONFIG_H -DHAVE_EXPAT_H -DHAVE_EXPAT arrows.c emit.c utils.c labels.c memory.c fontmetrics.c geom.c globals.c htmllex.c htmlparse.c htmltable.c ns.c pointset.c postproc.c routespl.c shapes.c splines.c colxlate.c psusershape.c input.c timing.c output.c
...
$(SRCDIR)/lib/dotgen/libdotgen-em.bc:
	cd $(SRCDIR)/lib/dotgen; $(EMCC) $(EMFLAGS) -o libdotgen-em.bc -I. -I.. -I../.. -I../../.. -I../common -I../gvc -I../pathplan -I../cdt -I../graph -DHAVE_CONFIG_H acyclic.c aspect.c class1.c class2.c cluster.c compound.c conc.c decomp.c dotinit.c dotsplines.c fastgr.c flat.c mincross.c position.c rank.c sameport.c

$(SRCDIR)/lib/neatogen/libneatogen-em.bc:
	cd $(SRCDIR)/lib/neatogen; $(EMCC) $(EMFLAGS) -o libneatogen-em.bc -I. -I.. -I../.. -I../../.. -I../common -I../gvc -I../pack -I../ortho -I../rbtree -I../sfdpgen -I../sparse -I../pathplan -I../cdt -I../graph -DHAVE_CONFIG_H adjust.c circuit.c edges.c geometry.c heap.c hedges.c info.c neatoinit.c legal.c lu.c matinv.c memory.c poly.c printvis.c site.c solve.c neatosplines.c stuff.c voronoi.c stress.c kkutils.c matrix_ops.c embed_graph.c dijkstra.c conjgrad.c pca.c closest.c bfs.c constraint.c quad_prog_solve.c smart_ini_x.c constrained_majorization.c opt_arrangement.c overlap.c call_tri.c compute_hierarchy.c delaunay.c multispline.c
...
$(SRCDIR)/plugin/dot_layout/libgvplugin_dot_layout-em.bc:
	cd $(SRCDIR)/plugin/dot_layout; $(EMCC) $(EMFLAGS) -o libgvplugin_dot_layout-em.bc -I. -I.. -I../.. -I../../.. -I../../lib -I../../lib/common -I../../lib/gvc -I../../lib/pathplan -I../../lib/cdt -I../../lib/graph -DHAVE_CONFIG_H gvplugin_dot_layout.c gvlayout_dot_layout.c

$(SRCDIR)/plugin/neato_layout/libgvplugin_neato_layout-em.bc:
	cd $(SRCDIR)/plugin/neato_layout; $(EMCC) $(EMFLAGS) -o libgvplugin_neato_layout-em.bc -I. -I.. -I../.. -I../../.. -I../../lib -I../../lib/common -I../../lib/gvc -I../../lib/pathplan -I../../lib/cdt -I../../lib/graph -DHAVE_CONFIG_H gvplugin_neato_layout.c gvlayout_neato_layout.c
